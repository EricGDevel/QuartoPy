...
def evaluate(state: GameState) -> Tuple[int, List[int]]:
    ...
    for row in rows:
        ...
        if num_of_pieces == length:
            return MAX_SCORE, winning_attributes
...
def calculate_row_attributes(row: List[Piece]) -> Tuple[List[Union[bool, int]], int]:
    ...
    for piece in row:
        if piece is None:
            continue
        attributes = list(piece.attributes) if num_of_pieces == 0 else \
            [att1 if att1 == att2 else NO_ATTRIBUTES for att1, att2 in zip(attributes, piece.attributes)]
        num_of_pieces += 1
...
def get_playable_moves(option: GameState, pieces_set: Set[Piece], i: int, j: int,
    ...
    if not options:
        options = [Option(option.board, tuple(pieces_set)[0], i, j)]
    return options
...
def iterative_deepening(board: GameState, piece: Piece, pieces_set: Set[Piece], sign: int,
                        alpha: float, beta: float, max_depth: int) -> Union[int, Option]:
    """
    :param board:       The current GameState

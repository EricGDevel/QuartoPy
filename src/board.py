...
class Board(GridLayout):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.first_move()
        Logger.debug("Setup: Initialised new {game_mode} Board".format(game_mode=self.game_mode))
    ...
    def first_move(self) -> None:
        ...
        if self.game_mode == GameMode.single_player:
            if self.first_player == Player.human:
    ...
    def computer_move(self) -> None:
        ...
        :return:    None
        """
        option = minimax(self, self.pieces_bar.confirmed.piece, self.pieces_bar.pieces_set, self.depth)
        i, j = option.index
    ...
    def get_current_player_str(self) -> str:
        return self.current_player.name.title() if self.game_mode == GameMode.single_player else \
            f'Player {self.current_player.value}'
    ...
    def confirm(self, touch) -> None:
        ...
        if isinstance(touch, Piece):
            ...
            for cell in self.widgets:
                if cell.piece == touch:
                    self.confirmed = cell
                    cell.color = Color.CONFIRMED_TINT.value
                    break
            if self.confirmed is None:
                raise ValueError('Invalid piece to confirm.')
            self.pieces_set.remove(touch)
        else:
            if self.selected is not None and self.confirmed is None:
                ...
                self.pieces_set.remove(self.selected.piece)
                self.selected = None
